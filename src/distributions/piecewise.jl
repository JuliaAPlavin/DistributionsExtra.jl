...
PiecewiseUniform((;edges, probs)::NamedTuple{(:edges,:probs)}) = PiecewiseUniform(edges, DiscreteNonParametric(Base.OneTo(length(edges) - 1), probs))
PiecewiseUniform((;edges, relprobs)::NamedTuple{(:edges,:relprobs)}) = PiecewiseUniform(;edges, probs=relprobs ./ sum(relprobs))
PiecewiseUniform((;edges, reldensities)::NamedTuple{(:edges,:reldensities)}) = PiecewiseUniform(;edges, relprobs=reldensities .* diff(edges))
...
function Distributions.pdf(d::PiecewiseUniform, x::Real)
    first(d.edges) ≤ x ≤ last(d.edges) || return zero(promote_type(eltype(d.edges), typeof(x)))
