...
@testitem "to intervals" begin
    ...
    @test pred_to_intervals(@optic(_^3 ∈ IntervalUnion((-1..8, 2..27)))) == -1..3
    @test pred_to_intervals(!@optic(_^3 > 27)) == IntervalUnion(Interval{:open,:closed}(-Inf, 3))
    @test pred_to_intervals(@optic(abs(_) > 5)) == IntervalUnion((Interval{:open, :closed}(5, Inf), Interval{:closed, :open}(-Inf, -5)))
